...
export function DashboardPage() {
  ...
  const statusText = allOperational ? 'SYSTEM NOMINAL' : someError ? 'CRITICAL ALERT' : 'SYSTEM WARNING';
  const statusColor = allOperational ? 'bg-status-ok' : someError ? 'bg-status-error' : 'bg-status-warning';
    ...
    const pushAlerts = (service: any, name: string) => {
       if (service.data?.status?.health) {
          service.data.status.health.forEach((h: any) => {
             list.push({ id: `${name}-${h.message}`, service: name, message: h.message, type: h.type || 'warning' });
          });
       }
    };
    ...
    return (
      <div 
        ...
        className={cn(
          ...
          isEditMode ? "cursor-grab active:cursor-grabbing scale-[0.98]" : "cursor-default",
          holdingId === id && "scale-[0.96] opacity-70 border-accent shadow-[0_0_20px_rgba(0,180,216,0.2)]"
        )}
      >
        {isStatCard ? (
        ...
        {isEditMode && (
          <button 
            ...
            className="absolute -top-2 -right-2 p-1.5 bg-red-500 rounded-full text-white opacity-0 group-hover:opacity-100 transition-all z-60 shadow-xl hover:scale-110 active:scale-95 border-2 border-[#141414]"
          >
            <X size={12} />
          </button>
        )}
  ...
  const allPossibleWidgets = [
    { id: 'downlink', label: 'Downlink Speed', icon: Download, color: 'text-accent' },
    { id: 'uplink', label: 'Uplink Speed', icon: Upload, color: 'text-prowlarr' },
    { id: 'storage', label: 'Storage Status', icon: Tv, color: 'text-sonarr' },
    { id: 'fleet', label: 'Fleet Monitor', icon: Film, color: 'text-radarr' },
    { id: 'activity', label: 'Operational Pulse', icon: ActivityIcon, color: 'text-white' },
    { id: 'efficiency', label: 'System Efficiency', icon: Zap, color: 'text-accent' },
    { id: 'security', label: 'Security Status', icon: ShieldCheck, color: 'text-status-ok' },
    { id: 'storage_dist', label: 'Storage Array', icon: Database, color: 'text-white' },
    { id: 'quality_dist', label: 'Quality Layout', icon: Layers, color: 'text-white' },
    { id: 'health_dist', label: 'Health Distribution', icon: HeartPulse, color: 'text-status-error' },
  ];
  ...
  return (
    <div className="space-y-12 pt-8 md:pt-12 pb-24 max-w-[1600px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-1000 px-4 md:px-8">
      ...
      <div className={cn("relative transition-all duration-500 min-h-[500px] rounded-3xl", isEditMode && "bg-surface/30 bg-grid-dots ring-1 ring-border p-6 shadow-2xl deck-edit-active")}>
        ...
        {layouts && typeof layouts === 'object' && !Array.isArray(layouts) && (layouts as any).lg ? (
          <ResponsiveGridLayout 
            ...
            onLayoutChange={(_current: any, allLayouts: any) => setLayouts(allLayouts)} 
            margin={[24, 24]}
