import { useState } from 'react';
import { useLibrary } from '../hooks/useLibrary';
import { LibraryControls } from '../components/library/LibraryControls';
import { MediaCard } from '../components/library/MediaCard';
import { MediaDetailSheet } from '../components/library/MediaDetailSheet';
import { MediaGridSkeleton } from '../components/common/LoadingSkeleton';
import { EmptyState, ErrorState } from '../components/common/States';
import { useBulkSelection } from '../hooks/useBulkSelection';
import { BulkCommandBar } from '../components/library/BulkCommandBar';
import { sonarrService } from '../services/sonarr.service';
import { useSettingsStore } from '../stores/settings.store';
import { toast } from 'sonner';

export function SonarrPage() {
  const { sonarr: sonarrSettings } = useSettingsStore();
  const { 
    items, isLoading, isError, totalCount, filteredCount,
    search, setSearch, filter, setFilter, refetch 
  } = useLibrary('sonarr');

  const [selectedItem, setSelectedItem] = useState<any>(null);
  const [sheetOpen, setSheetOpen] = useState(false);
  const [isUpdating, setIsUpdating] = useState(false);

  const { 
    selectedIds, 
    isSelected, 
    isSelectionMode, 
    toggleId, 
    clearSelection 
  } = useBulkSelection<number>();

  const handleItemClick = (item: any) => {
    setSelectedItem(item);
    setSheetOpen(true);
  };

  const handleBulkMonitor = async (monitored: boolean) => {
    if (!sonarrSettings.enabled) return;
    setIsUpdating(true);
    try {
      await sonarrService.bulkUpdateSeries(
        sonarrSettings.baseUrl,
        sonarrSettings.apiKey,
        selectedIds,
        { monitored }
      );
      toast.success(`Updated ${selectedIds.length} series`);
      refetch();
    } catch (err) {
      toast.error('Failed to update series');
    } finally {
      setIsUpdating(false);
    }
  };

  const handleBulkDelete = async () => {
    if (!sonarrSettings.enabled) return;
    setIsUpdating(true);
    try {
      await sonarrService.bulkDeleteSeries(
        sonarrSettings.baseUrl,
        sonarrSettings.apiKey,
        selectedIds,
        true
      );
      toast.success(`Deleted ${selectedIds.length} series`);
      clearSelection();
      refetch();
    } catch (err) {
      toast.error('Failed to delete series');
    } finally {
      setIsUpdating(false);
    }
  };

  return (
    <div className="p-8 max-w-7xl mx-auto space-y-12 pb-24 animate-in fade-in duration-700">
      <div className="flex flex-col gap-2">
        <h1 className="text-4xl font-black text-foreground tracking-tighter uppercase italic">
          Series <span className="text-sonarr underline decoration-4 underline-offset-4">Fleet</span>
        </h1>
        <p className="text-muted-foreground font-medium tracking-tight italic uppercase text-[11px]">
          Aggregated series library and episode statistics across your local network
        </p>
      </div>

      <LibraryControls 
        search={search}
        onSearchChange={setSearch}
        filter={filter}
        onFilterChange={setFilter}
        onRefresh={refetch}
        isLoading={isLoading}
        totalCount={totalCount}
        filteredCount={filteredCount}
      />

      {isError && (
        <ErrorState 
          message="Failed to load series library. Please verify your Sonarr connection and API key in settings." 
          onRetry={refetch} 
        />
      )}

      {isLoading && items.length === 0 ? (
        <MediaGridSkeleton />
      ) : items.length === 0 && !isError ? (
        <EmptyState 
          message="No series found matching your criteria. Try adjusting your filters or search term." 
        />
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
          {items.map((series: any) => (
            <MediaCard 
              key={series.id} 
              item={series} 
              service="sonarr" 
              isSelected={isSelected(series.id)}
              isSelectionMode={isSelectionMode}
              onToggleSelection={toggleId}
              onClick={() => handleItemClick(series)} 
            />
          ))}
        </div>
      )}

      <MediaDetailSheet 
        item={selectedItem}
        service="sonarr"
        open={sheetOpen}
        onOpenChange={setSheetOpen}
      />

      <BulkCommandBar 
        count={selectedIds.length}
        onClear={clearSelection}
        onMonitor={handleBulkMonitor}
        onDelete={handleBulkDelete}
        isUpdating={isUpdating}
      />
    </div>
  );
}
