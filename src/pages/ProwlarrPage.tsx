    ...
    <div className="p-8 max-w-7xl mx-auto space-y-12 pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <div className="flex flex-col gap-2">
            <h1 className="text-4xl font-black text-foreground tracking-tighter uppercase italic">
              Fleet <span className="text-prowlarr">Management</span>
            </h1>
            <p className="text-foreground/60 font-medium tracking-tight italic uppercase text-[11px]">
              Aggregated tracker health and synchronization bridge
            </p>
          </div>
          <p className="text-foreground/60 font-medium tracking-tight">
            ...
            trackers across your Arr ecosystem
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            className="bg-foreground/5 hover:bg-foreground/10 text-foreground font-bold gap-2 px-6 h-12"
          ...
          <Button
            className="bg-prowlarr hover:bg-prowlarr/80 text-foreground font-black uppercase tracking-widest gap-2 px-8 h-12 shadow-lg shadow-prowlarr/20"
          ...
          <Card className="bg-status-error/10 border-status-error/20 overflow-hidden relative">
            ...
            <CardContent className="p-6 flex items-start gap-5">
              ...
              <div className="flex-1 space-y-1">
                ...
                <div className="space-y-2">
                  {healthIssues.map((issue, idx) => (
                    <p
                      key={idx}
                      className="text-foreground/60 text-xs font-medium leading-relaxed"
                    >
                      {issue.message}
                    </p>
                  ))}
              ...
              <Button
                ...
                onClick={() => window.open(prowlarr.baseUrl, "_blank")}
              >
                FIX IN PROWLARR
              </Button>
            </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        />
        <div className="bg-foreground/5 rounded-3xl p-6 border border-foreground/5 flex flex-col justify-center gap-1 group cursor-pointer hover:bg-foreground/10 transition-colors">
          <span className="text-[10px] uppercase font-black tracking-[0.2em] text-foreground/20">
            Global Search
          </span>
          <div className="flex items-center justify-between">
            <span className="text-lg font-black text-foreground/60 group-hover:text-foreground transition-colors">
              Launch Query
            </span>
            <Search className="h-5 w-5 text-foreground/20 group-hover:text-accent group-hover:animate-pulse transition-colors" />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 px-2.5 py-1 bg-foreground/5 rounded-full border border-foreground/5">
          ...
          <Button
            variant="ghost"
            size="sm"
            className="text-foreground/20 hover:text-foreground gap-2 font-bold"
            onClick={() => refetchFleet()}
          >
            <RefreshCw
              className={`h-3 w-3 ${fleetLoading ? "animate-spin text-accent" : ""}`}
            />
            FORCE POLLING
          </Button>
        </div>
        ...
        {fleetLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="h-44 bg-foreground/5 animate-pulse rounded-2xl border border-foreground/5"
              />
            ))}
          </div>
        ) : (
...
function StatCard({
  ...
  return (
    <div className="bg-card border border-foreground/5 rounded-2xl p-6 relative overflow-hidden group hover:border-foreground/10 transition-all shadow-xl">
      ...
      <div className="absolute bottom-0 left-0 w-full h-px bg-linear-to-r from-transparent via-foreground/5 to-transparent shadow-[0_-2px_10px_hsl(var(--foreground)/0.05)]" />
    </div>
  );
}
