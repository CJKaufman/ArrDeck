...
export function TorrentTableRow({
  ...
  const getStateColor = (state: string) => {
    ...
    if (
      state.includes("uploading") ||
      state.includes("forcedUP") ||
      state.includes("stalledUP") ||
      (state.includes("checking") && torrent.progress === 1)
    ) {
      return "spectral-indigo bg-indigo-300/10 border-indigo-300/30";
    }
    // Waiting / Paused Cluster
    if (state.includes("paused")) {
      return "spectral-amber bg-status-warning/10 border-status-warning/30";
    }
    // Stalled / Queued Cluster
    if (state.includes("stalled") || state.includes("queued")) {
      return "bg-foreground/5 text-foreground/40 border border-foreground/10";
    }
    // Error Cluster
    if (state.includes("error") || state.includes("missingFiles")) {
      return "spectral-red bg-status-error/10 border-status-error/30";
    }
    return "bg-foreground/5 text-foreground/40 border border-foreground/5";
  };
  ...
  return (
    <tr
      ...
      className={cn(
        "group border-b border-foreground/3 hover:bg-foreground/2 cursor-pointer transition-colors duration-150",
    ...
    >
      <td className="py-2 pl-4 w-[40px]" onClick={(e) => e.stopPropagation()}>
        <input
          ...
          className="h-3.5 w-3.5 rounded border border-foreground/20 bg-foreground/5 checked:bg-accent checked:border-accent transition-all appearance-none cursor-pointer relative checked:before:content-['✓'] checked:before:absolute checked:before:inset-0 checked:before:flex checked:before:items-center checked:before:justify-center checked:before:text-[10px] checked:before:text-foreground"
        />
      </td>

      <td className="py-2 px-2 text-[10px] font-bold text-foreground/30 tabular-nums whitespace-nowrap">
      ...
      <td className="py-2 px-3 max-w-[300px]">
        <div className="flex items-center gap-2 overflow-hidden">
          <CircleDot
            className={cn(
              "h-2 w-2 shrink-0 animate-pulse",
              torrent.progress === 1 ? "text-status-ok" : "text-accent",
            )}
          />
          <span className="text-[11px] font-black tracking-tight text-foreground/90 truncate group-hover:text-foreground transition-colors">
            {torrent.name}
          </span>
        </div>
      </td>

      <td className="py-2 px-3 text-[10px] font-bold text-foreground/50 tabular-nums text-right">
      ...
      <td className="py-2 px-3 w-[150px]">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-black tabular-nums text-foreground/40 min-w-[30px]">
      ...
      <td className="py-2 px-3 text-[10px] font-bold text-foreground/40 tabular-nums text-right">
        {torrent.ratio.toFixed(2)}
      </td>

      <td className="py-2 px-3 text-right">
        <span className="text-[10px] font-bold text-foreground/20 uppercase tracking-widest italic group-hover:text-foreground/40 transition-colors">
        ...
        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          {torrent.state.includes("paused") ? (
            <button
              ...
              className="h-6 w-6 rounded bg-accent/10 border border-accent/20 flex items-center justify-center hover:bg-accent/20 transition-colors"
            >
            ...
            <button
              ...
              className="h-6 w-6 rounded bg-foreground/5 border border-foreground/10 flex items-center justify-center hover:bg-foreground/10 transition-colors"
            >
          ...
          <button
            ...
            className="h-6 w-6 rounded bg-status-error/10 border border-status-error/20 flex items-center justify-center hover:bg-status-error/20 transition-colors"
          >
