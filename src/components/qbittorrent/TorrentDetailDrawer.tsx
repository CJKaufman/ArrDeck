...
export function TorrentDetailDrawer({ torrent }: Props) {
  ...
  return (
    <SheetContent className="sm:max-w-2xl bg-card/95 border-l border-foreground/5 p-0 flex flex-col backdrop-blur-3xl shadow-2xl">
      <SheetHeader className="p-8 pb-4 relative overflow-hidden bg-gradient-to-b from-foreground/5 to-transparent">
        <div className="flex items-center justify-between mb-4">
          <div
            className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border ${
              torrent.state.includes("downloading")
                ? "bg-accent/10 text-accent border-accent/20"
                : torrent.state.includes("uploading") ||
                    torrent.state.includes("seeding")
          ...
          <span className="text-[10px] font-mono text-foreground/20 select-all uppercase tracking-tighter">
            {torrent.hash}
          </span>
        </div>
        <SheetTitle className="text-2xl font-black text-foreground tracking-tight leading-tight pr-12 line-clamp-2">
          {torrent.name}
        </SheetTitle>
      </SheetHeader>

      <Tabs defaultValue="info" className="flex-1 flex flex-col">
        <div className="px-8 border-b border-foreground/5">
          <TabsList className="bg-transparent h-12 p-0 gap-8 justify-start w-full">
            <TabsTrigger
              value="info"
              className="relative h-12 rounded-none bg-transparent border-b-2 border-transparent data-[state=active]:border-accent data-[state=active]:bg-transparent data-[state=active]:text-foreground text-foreground/40 font-bold transition-all px-0"
            >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard
                icon={<Share2 className="h-4 w-4 text-status-ok" />}
                label="Ratio"
              ...
              <StatCard
                ...
                value={
                  torrent.eta === 8640000
                    ? "∞"
                    : format(new Date(Date.now() + torrent.eta * 1000), "HH:mm")
                }
              />
              <StatCard
                icon={<Database className="h-4 w-4 text-muted-foreground" />}
                label="Total"
                value={prettyBytes(torrent.size)}
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
              ...
              <div className="bg-foreground/5 rounded-2xl p-6 border border-foreground/5 space-y-4">
                <h4 className="text-[10px] uppercase font-black tracking-[0.2em] text-foreground/20">
                  Transfer Health
                </h4>
            ...
            {props && (
              <div className="space-y-4 border-t border-foreground/5 pt-8">
                <h4 className="text-[10px] uppercase font-black tracking-[0.2em] text-foreground/20">
                  Client Metadata
                </h4>
                <div className="grid grid-cols-2 gap-6">
                  <DetailItem
                    label="Wasted Data"
                    value={prettyBytes(props.total_wasted)}
                    color="text-status-error"
                  />
                  <DetailItem
                    label="Active Time"
                    value={`${(props.time_elapsed / 3600).toFixed(1)} hours`}
                  />
                </div>
              </div>
            )}
          ...
          >
            {filesLoading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <span className="text-xs text-foreground/20 uppercase tracking-widest font-black">
                  Scanning File Tree
                </span>
              </div>
            ) : (
              <div className="space-y-3">
                {files?.map((file) => (
                  <div
                    key={file.index}
                    className="bg-foreground/5 border border-foreground/5 rounded-xl p-4 transition-colors hover:bg-foreground/10 group"
                  >
                    <div className="flex items-start justify-between mb-3 gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-foreground line-clamp-1 break-all pr-4">
                          {file.name}
                        </span>
                      </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex-1">
                        <div className="flex justify-between text-[10px] uppercase tracking-wider mb-1 px-1">
                          ...
                          <span
                            className={`${file.progress === 1 ? "text-status-ok" : "text-accent"} font-black`}
                          >
                        ...
                        <Progress
                          ...
                          className={`h-1 cursor-default ${file.progress === 1 ? "bg-status-ok/20" : ""}`}
                        />
                      </div>
                    </div>
                  </div>
            ...
            {trackersLoading ? (
              ...
              <div className="space-y-3">
                {trackers?.map((tracker) => (
                  <div
                    key={tracker.url}
                    className="bg-card border border-foreground/5 rounded-xl p-4 flex items-center justify-between gap-6"
                  >
                    <div className="flex flex-col gap-1 min-w-0">
                      <span className="text-xs font-mono text-foreground/60 truncate tracking-tight">
                        {tracker.url}
                      </span>
...
function StatCard({
  ...
  return (
    <div className="bg-foreground/5 border border-foreground/5 rounded-2xl p-4 flex flex-col gap-3">
      {icon}
      <div>
        <p className="text-[9px] uppercase font-black tracking-[0.2em] text-foreground/20 leading-none mb-1.5">
          {label}
        </p>
...
function DetailItem({
  ...
  return (
    <div className="space-y-2">
      <span className="text-[10px] uppercase font-black tracking-[0.2em] text-foreground/10">
        {label}
      </span>
      {isPath ? (
        <div className="flex items-center gap-3 p-3 bg-foreground/5 rounded-xl border border-foreground/5">
          <HardDrive className="h-3.5 w-3.5 text-foreground/20" />
        ...
        <p
          className={`text-sm font-bold tracking-tight ${color} ${isStatus ? "uppercase tracking-widest" : ""}`}
        >
          {value}
        </p>
      )}
...
function HealthBar({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <Progress
        value={percentage}
        className={`h-1 bg-foreground/5 ${color}/20`}
      />
    </div>
  );
}
