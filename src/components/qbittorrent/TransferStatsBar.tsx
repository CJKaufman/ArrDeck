        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className={`flex items-center gap-2 px-2 py-1 rounded transition-all ${
              isToggling || !transfer ? 'opacity-50 grayscale' : 'hover:bg-surface/10'
            }`}
          ...
          >
            ...
            <span className={`text-[10px] tracking-tight ${
              !transfer ? 'text-muted-foreground/20' : transfer.use_alt_speed_limits ? 'text-[#EF4444]' : 'text-[#22C55E]'
            }`}>
          ...
          <div className="flex items-center gap-5 font-mono">
            <div className="flex items-center gap-2 group">
              ...
              <span className="text-foreground drop-shadow-sm min-w-[70px]">{prettyBytes(transfer?.dl_info_speed || 0)}/s</span>
            </div>
            <div className="flex items-center gap-2 group">
              <Upload className="h-3.5 w-3.5 text-muted-foreground/40 group-hover:text-muted-foreground transition-colors" />
              <span className="text-foreground drop-shadow-sm min-w-[70px]">{prettyBytes(transfer?.up_info_speed || 0)}/s</span>
            </div>
          </div>
        </div>
        
        <div className="hidden lg:flex items-center gap-4 border-l border-border/40 pl-6 ml-2">
          <span className="text-muted-foreground/30 text-[9px] uppercase tracking-[0.2em] font-black">Session</span>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5 px-3 py-1 rounded bg-surface border border-border">
            ...
            <span className="text-[10px] text-foreground/70 uppercase tracking-wider">qBittorrent</span>
