...
export function ReleaseSearchDialog({
  ...
  const {
    ...
    queryFn: () => {
      if (service === "sonarr")
        return sonarrService.getReleases(
          sonarr.baseUrl,
          sonarr.apiKey,
          mediaId,
          episodeId,
  ...
  const handleDownload = async (release: any) => {
    ...
    try {
      if (service === "sonarr") {
        await sonarrService.downloadRelease(
          sonarr.baseUrl,
          sonarr.apiKey,
          release.guid,
          release.indexerId,
        ...
        await radarrService.downloadRelease(
          radarr.baseUrl,
          radarr.apiKey,
          release.guid,
          release.indexerId,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] lg:max-w-[1250px] max-h-[90vh] overflow-hidden flex flex-col p-10 bg-surface border-foreground/10 shadow-[0_0_100px_rgba(0,0,0,0.6)] rounded-3xl">
        {/* Decorative Backdrop */}
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <Badge className="bg-accent/20 text-accent font-black text-[10px] uppercase tracking-widest px-2 py-0 border-none">
                  Interactive Search
                </Badge>
                <div className="h-1 w-1 rounded-full bg-foreground/20" />
                <span className="text-xs font-bold text-muted-foreground uppercase tracking-tighter">
                  Scanning all indexers
                </span>
              </div>
              <DialogTitle className="text-4xl font-black tracking-tight text-foreground drop-shadow-lg">
                Releases for{" "}
                <span className="text-accent drop-shadow-[0_0_15px_rgba(0,184,212,0.4)]">
                  {title}
                </span>
              </DialogTitle>
            </div>
            ...
            {!isLoading && (
              <Button
                ...
                size="sm"
                onClick={() => refetch()}
                className="rounded-xl border-foreground/10 hover:bg-foreground/5 font-bold h-10 px-4"
              >
                Refresh Results
              </Button>
            )}
          ...
          <div className="flex-1 flex flex-col items-center justify-center space-y-6 relative z-10">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground animate-pulse">
                Consulting your providers for the best available files...
              </p>
            </div>
          </div>
        ) : isError ? (
          <div className="flex-1 flex flex-col items-center justify-center space-y-6 text-destructive">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-sm opacity-70">
                Failed to fetch release results. Please check your service
                configuration.
              </p>
            </div>
            <Button
              variant="outline"
              onClick={() => refetch()}
              className="rounded-xl border-foreground/10 font-bold"
            >
              Try Global Scan Again
            </Button>
          </div>
        ) : (
              ...
              {releases?.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-32 text-muted-foreground space-y-4">
                  ...
                  <p className="text-lg font-bold opacity-30">
                    No matching releases found across your indexed trackers.
                  </p>
                </div>
              ) : (
                    ...
                    <div
                      ...
                      className={`p-5 rounded-2xl border transition-all duration-300 group ${
                        isRejected
                          ...
                          : "bg-foreground/[0.04] border-foreground/10 hover:border-accent/40 hover:bg-foreground/[0.06] shadow-xl"
                      }`}
                    >
                      <div className="flex items-center gap-6">
                        {/* Score Badge */}
                        <div
                          className={`shrink-0 w-16 h-16 rounded-xl border flex flex-col items-center justify-center transition-transform group-hover:scale-105 ${scoreColor}`}
                        >
                        ...
                        <div className="flex-1 min-w-0 flex flex-col gap-2">
                          <div className="flex items-center gap-3">
                            <h4
                              className={`text-lg font-black truncate tracking-tight transition-colors drop-shadow-sm ${!isRejected ? "text-foreground group-hover:text-accent" : "text-foreground/60"}`}
                            >
                              {release.title}
                            </h4>
                            {release.infoUrl && (
                          ...
                          <div className="flex flex-wrap items-center gap-y-2 gap-x-6 text-[11px] font-bold uppercase tracking-wider text-foreground/70">
                            <div className="flex items-center gap-2">
                              <span className="bg-accent/20 text-accent px-2 py-0.5 rounded-md border border-accent/20 drop-shadow-sm">
                                {release.indexer}
                              </span>
                            </div>
                            <span className="flex items-center gap-1.5">
                              <HardDrive className="h-3.5 w-3.5 opacity-70" />{" "}
                              {prettyBytes(release.size)}
                            </span>
                            <span className="flex items-center gap-1.5">
                            ...
                            <span
                              className={`px-2 py-0.5 rounded border border-foreground/20 drop-shadow-sm ${!isRejected ? "text-foreground/90 bg-foreground/10" : "opacity-60"}`}
                            >
                          ...
                          {isRejected && (
                            <div className="mt-2 flex flex-wrap gap-2">
                              {release.rejections.map(
                                (rej: string, idx: number) => (
                                  <Badge
                                    key={idx}
                                    variant="ghost"
                                    className="text-[10px] text-status-error bg-status-error/10 hover:bg-status-error/20 border-status-error/20 px-2 py-1 flex items-center gap-1.5 rounded-lg font-black uppercase tracking-wider shadow-sm"
                                  >
                        ...
                        {/* Action Section */}
                        <div className="shrink-0 pl-4 border-l border-foreground/5 h-16 flex items-center">
                          <Button
                            className={`h-12 px-6 rounded-xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 shadow-lg ${
                              isRejected
                                ? "bg-foreground/5 hover:bg-foreground/10 text-foreground/40 border-foreground/5"
                                : "bg-status-ok hover:bg-status-ok/80 text-foreground shadow-status-ok/20"
                            }`}
