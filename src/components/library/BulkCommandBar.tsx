...
export function BulkCommandBar({
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-12 duration-500">
      <div className="bg-[#0B0C0E]/90 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] px-6 py-4 flex items-center gap-8 min-w-[600px]">
        {/* Count Indicator */}
        <div className="flex flex-col border-r border-white/5 pr-8">
          <span className="text-[10px] font-black uppercase tracking-widest text-white/30 italic">Fleet Selected</span>
          <div className="flex items-baseline gap-1">
            <span className="text-3xl font-black italic text-white tracking-tighter leading-none">{count}</span>
            <span className="text-[11px] font-bold text-accent italic uppercase">Units</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="flex bg-white/5 p-1 rounded-xl gap-1">
            <Button
              ...
              disabled={isUpdating}
              className="h-9 px-4 hover:bg-white/5 text-white/60 hover:text-white font-black italic uppercase tracking-tighter text-[11px] gap-2"
            >
          ...
          {profiles && onProfileChange && (
            <div className="flex-1 max-w-[150px]">
               <Button
                  size="sm"
                  variant="outline"
                  className="w-full border-white/5 bg-white/5 text-white/60 font-black italic uppercase tracking-tighter text-[11px] gap-2 justify-between"
                  disabled={isUpdating}
               >
        ...
        <div className="flex items-center gap-4 border-l border-white/5 pl-8">
          ...
          <Button
            ...
            variant="ghost"
            onClick={onClear}
            className="h-10 w-10 text-white/20 hover:text-white hover:bg-white/5 rounded-full transition-all"
          >
            <X className="h-5 w-5" />
          </Button>
        </div>
