...
export const sonarrService = {
  async getQueue(baseUrl: string, apiKey: string): Promise<UnifiedQueueItem[]> {
    ...
    const res = await client.get(
      "/api/v3/queue?includeUnknownSeriesItems=true",
    );
    ...
    return items.map((item: any): UnifiedQueueItem => {
      ...
      return {
        ...
        trackedDownloadStatus: item.trackedDownloadStatus || "ok",
        statusMessages: item.statusMessages || [],
        needsImport: item.trackedDownloadState === "importPending",
      };
    });
  },
  ...
  async deleteFromQueue(
    ...
    const client = createApiClient(baseUrl, apiKey);
    await client.delete(
      `/api/v3/queue/${id}?removeFromClient=${removeFromClient}&blocklist=false`,
    );
  },
  ...
  async getHistory(baseUrl: string, apiKey: string, pageSize: number = 100) {
    const client = createApiClient(baseUrl, apiKey);
    const res = await client.get(
      `/api/v3/history?pageSize=${pageSize}&sortKey=date&sortDir=desc`,
    );
    return res.data;
  },
  ...
  async getManualImportItems(
    ...
    const client = createApiClient(baseUrl, apiKey);
    let url = `/api/v3/manualimport?downloadId=${encodeURIComponent(downloadId)}&filterExistingFiles=true`;
  ...
  async processManualImport(
    ...
    const client = createApiClient(baseUrl, apiKey);
    await client.post("/api/v3/manualimport", { files, importMode });
  },
};
